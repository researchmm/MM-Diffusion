...
class AudioCLIP(CLIP):
    ...
    def __init__(self,
        ...
        if isinstance(self.pretrained, str):
            print(f"Prepare eval model: load {self.pretrained} to AudioCLIP")
    ...
    def forward(self,
        ...
        if audio is not None:
            ...
            raw_audio_features = self.encode_audio(audio)
            audio_features = raw_audio_features / raw_audio_features.norm(dim=-1, keepdim=True)

        if video is not None:
            b, f, c, h, w = video.shape
            images = rearrange(video, "b f c h w -> (b f) c h w")
            raw_image_features = self.encode_image(images)
            image_features = raw_image_features / raw_image_features.norm(dim=-1, keepdim=True)

            raw_video_features = rearrange(raw_image_features, "(b f) c-> b f c", b=b)
            raw_video_features = raw_video_features.mean(dim=1)

            video_features = rearrange(image_features, "(b f) c-> b f c", b=b)
        ...
        features: ClipFeatures = (raw_audio_features, raw_video_features, text_features)

        logit_scale_ai = torch.clamp(self.logit_scale_ai.exp(), min=1.0, max=100.0)
        logit_scale_at = torch.clamp(self.logit_scale_at.exp(), min=1.0, max=100.0)
        logit_scale_it = torch.clamp(self.logit_scale.exp(), min=1.0, max=100.0)

        logits_audio_video = None
        logits_audio_text = None
        ...
        if (audio_features is not None) and (video_features is not None):
            ...
            logits_audio_video = rearrange(logits_audio_image, "a (b f) -> a b f", b = logits_audio_image.shape[0])
            logits_audio_video = logits_audio_video.mean(dim=-1)

        if (audio_features is not None) and (text_features is not None):
            logits_audio_text = logit_scale_at * audio_features @ text_features.T

        if (video_features is not None) and (text_features is not None):
            logits_video_text = logit_scale_it * video_features @ text_features.T

        logits: ClipLogits = (logits_audio_video, logits_audio_text, logits_video_text)
